    ...
    if (!m_langs.empty()) {
        for (size_t i = 0; i < m_langs.size(); ++i) {
            if (m_langs.at(i).first == langCode) {
                ...
                qDebug("Found index for language \"%s\" at %i in model with %u items.", qUtf8Printable(langCode), idx, m_periods.size());
                break;
            }
        }
    }
...
#ifdef QT_DEBUG
    if (idx < 0) {
        qDebug("Did not find index for language \"%s\" in model with %u items.", qUtf8Printable(langCode), m_periods.size());
    }
#endif
